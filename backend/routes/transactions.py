...
def sync_all(session):
    ...
    summary = transaction_service.sync_all(session)
    session.commit()
...
@with_session
def update_transaction(session, transaction_id):
...
@with_session
def assign_transaction(session, transaction_id):
    ...
    transaction_service.set_assignment(session, transaction_id, budget_item_id)
    return jsonify({}), 201


@bp.delete("/api/transactions/<int:transaction_id>/assignment")
@with_session
def unassign_transaction(session, transaction_id):
    """Remove a transaction's explicit budget item assignment."""
    transaction_service.remove_assignment(session, transaction_id)
    return "", 204
